...
/*   computed: {
    propsToWatch() {
      return {
        largeKGVisible: this.largeKGVisible,
  ...
  methods: {
    defaultKG() {
      this.loading = true
      let _this = this
        getDefaultRelLinks(this.currentId)
          .then(({data}) => {
    ...
    defaultOrInit(){
      console.log("this.largeKGVisible是"+this.largeKGVisible);
      console.log("this.drawFlag是"+this.drawFlag);
      if (this.largeKGVisible) {
        if (this.drawFlag) {
          this.initKG();
        } else {
          this.defaultKG();
  ...
  watch: {
    ...
    // 监听 'currentId' prop 的变化
    currentId(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.defaultKG(newVal);
      }
    },

    // //同时监听 查看大图 和 查询 按钮
    largeKGVisible(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.defaultOrInit();
      }
    },
    drawFlag(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.defaultOrInit();
      }
    }
/*     propsToWatch: {
      handler(newValues, oldValues) {
        console.log("this.laerKGVisible"+this.largeKGVisible);
        console.log("this.drawFlag"+this.drawFlag);
        // 新旧值比较
        if (newValues.largeKGVisible !== oldValues.largeKGVisible && newValues.drawFlag === oldValues.drawFlag) {
          // 只有largeKGVisible变化
          this.defaultKG();
        } else if (newValues.largeKGVisible !== oldValues.largeKGVisible && newValues.drawFlag !== oldValues.drawFlag) {
          // largeKGVisible和drawFlag都变化
          this.initKG();
        }
      },
      deep: true
