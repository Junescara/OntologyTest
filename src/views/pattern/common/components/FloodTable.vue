    ...
    <div class="flood-table-below">
      ...
      >
        <template #default>
          ...
          <!--进度条,应该嵌在模态框中-->
          <el-dialog
            v-model="matchInnerDialogVisible"
            title="历史模式库匹配"
            append-to-body
            destroy-on-close
            @closed="matchDialogVisible = false"
          >
            <el-progress
        ...
        <template #footer>
          ...
          <el-button
            @click="innerDialogVisibleControl((matchInnerDialogVisible = true))"
            type="primary"
            >匹配</el-button
          >
        </template>
      ...
      <!--详细信息模态框-->
      <el-dialog v-model="detailVisible" title="场次详情信息">
...
import MatchModel from "./MatchModel.vue";
...
export default {
  ...
  data() {
    return {
      ...
      Features: [
        ...
        { name: "降雨趋势", value: 1 },
        { name: "面降雨总量", value: 1 },
        { name: "MaxIndex", value: 1 },
        { name: "降雨量级", value: 1 },
        { name: "格点最大小时雨量", value: 1 },
        { name: "最大格点累计雨量", value: 1 },
        { name: "蓄水流量", value: 1 },
      ],
      basinName: [
        {
  ...
  methods: {
    ...
    innerDialogVisibleControl() {
      ...
      console.log(matchID);
      //请求后端拿到匹配的结果
      postRequest("/match", { flood_id: matchID, weights: weights.toString() })
        .then((res) => {
...
.match-dialog .slider-demo-block {
  text-align: left;
}
</style>
