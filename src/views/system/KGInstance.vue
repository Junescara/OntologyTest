...
export default {
  ...
  methods: {
    ...
    getAllNodeLabels() {
      ...
      //     console.log(error);
      //   });
      //加一个limit限制，获得limit（字符串）标签下所有结点的标签
      aggregateApi.getNodeLimitedLabels(this.currentId,this.nodeLabelLimit)
        .then((response) => {
          this.nodeLabels = response.data.data.nodeLabels
        })
        .catch((error) => {
    ...
    initAddCard(){
      ...
      //根据本体库获取可增加的实例类型列表
      //此处limit需要传“流域水循环对象本体” ，因为调用的不是nodeLabels接口，而是getOntoList
      ontologyApi.getOntoList(0,this.currentOntoId,this.nodeLabelLimit_onto)
      .then((response) => {
    ...
    initDbInfo(){
      this.currentDbName = localStorage.getItem("instanceName") // 上个页面传过来的instanceName
